...
export default function GitHubStats() {
    ...
    return ( <
        section id = "github"
        className = "relative py-20 lg:py-32 bg-gradient-to-b from-pearl to-white" >
        <
        div className = "max-w-7xl mx-auto section-padding" >
        <
        SectionHeader subtitle = "GitHub Activity"
        ...
        <
        div ref = { ref }
        className = "space-y-8" > { /* Stats Cards */ } <
        ...
        <
        div className = "grid lg:grid-cols-2 gap-8" > { /* Contribution Graph */ } <
        motion.div initial = {
            { opacity: 0, x: -30 }
        }
        ...
        div className = "flex items-end justify-between gap-2 h-48" > {
            contributionData.map((day, index) => ( <
                ...
                <
                div className = "absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap" > { day.count }
                commits <
                /div> < /
                div > <
        ...
        <
        span className = "w-2 h-2 bg-royal-500 rounded-full" / > { contributionData.reduce((a, b) => a + b.count, 0) }
        commits <
        /span> < /
        div > <
        /motion.div>
        ...
        div className = "space-y-4" > {
            topLangs.map((lang, index) => ( <
                ...
                transition = {
                    { duration: 1, delay: 0.4 + index * 0.1 }
                }
